...
def recommend():
    ...
    # Step 1 & 2: Candidate Generation and AI Scoring
    candidates = []
    for fruit_name, details in fruit_database.items():
        try:
            for variety_name, variety_info in details.get('varieties', {}).items():
                ...
                state_match = False
                if isinstance(peak_states, dict): state_match = state in peak_states
                elif isinstance(peak_states, list): state_match = state in peak_states
                
                if state_match:
                    candidates.append({
                        "fruit_name": fruit_name, "variety": variety_name,
    ...
    for idx, candidate in candidates_df.iterrows():
        ...
        # --- THIS IS THE FIX ---
        # Ensure the score never goes above 1.0 after the boost.
        ...
        results.append({
            'fruit_name': fruit_name, 'variety': variety_name,
            'score': round(final_score, 4), 'image_url': image_url,
            'reason': reason, 'benefits': fruit_details.get('benefits', 'No description available.'),
            'health_tags': health_tags, 'matches_focus': matches_focus
        })

    # Step 4: Smart Sorting and De-duplication Logic
    if health_focus:
        results.sort(key=lambda r: (r['matches_focus'], r['score']), reverse=True)
    else:
    ...
    for rec in results:
        if rec['fruit_name'] not in displayed_fruits:
            del rec['matches_focus'] # Clean up the temporary key
